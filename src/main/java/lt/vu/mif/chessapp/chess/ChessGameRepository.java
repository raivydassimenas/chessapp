...
public class ChessGameRepository {
    ...
    public Optional<ChessGame> findById(Long id) {
        ...
        return jdbcTemplate.query(connection -> {
            ...
            if (rs.next()) {
                return Optional.of(new ChessGame(rs.getLong("id"), rs.getString("white_player"), rs.getString("black_player"), rs.getString("result"), rs.getTimestamp("created").toLocalDateTime(), rs.getTimestamp("updated").toLocalDateTime(), rs.getString("game_file")));
            } else {
    ...
    private static class ChessGameRowMapper implements RowMapper<ChessGame> {
        ...
        public ChessGame mapRow(ResultSet rs, int rowNum) throws SQLException {
            return new ChessGame(rs.getLong("id"), rs.getString("white_player"), rs.getString("black_player"), rs.getString("result"), rs.getTimestamp("created").toLocalDateTime(), rs.getTimestamp("updated").toLocalDateTime(), rs.getString("game_file"));
        }
    }
}